\ Note: The ANS Forth definition of what happens when you
\       use POSTPONE on words with different compilation and
\       runtime semantics is inconsistent. By this reason any
\       test for the "correct" implementation of POSTPONE
\       can be contradicted.
...
\ minor adaptations to MinForth by AKK 2000

\ This file is based on John Hayes' core.fr, which has
...
\ You have to load John Hayes' tester.fr and core.fr first
...
\ Anti-Bernd-Paysan-Test deleted  ;-) AKK
...
{ : PSQ4 [ postpone-S" XY" ] ; -> }
{ PSQ4 SWAP DROP -> 2 }
{ PSQ4 DROP DUP C@ SWAP CHAR+ C@ -> 88 89 }
